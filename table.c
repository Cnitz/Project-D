...
void tbl_print_row();
void tester();
Row* tbl_copy_row(Row* r);
void tbl_add_row(Table* tbl, Row* r);
...
//TODO: figure out how to copy row next pointer into new table. possible put into resample funciton.
Row* tbl_copy_row(Row* r){
    Row* ret = calloc(1, sizeof(Row));
    int fields = r->fields;
    ret->fields = fields;
    ret->data = calloc(fields, sizeof(Data));
    
    for(int i = 0; i < fields; i++){
        if(r->type[i] == 'S'){
            ret->data[i].c = calloc(strlen(r->data[i].c)+1, sizeof(char));
            strcpy(ret->data[i].c, r->data[i].c);
            continue;
        }
        else
        {
            ret->data[i].d = r->data[i].d;
        }
            
    }
    ret->type = calloc(fields+1, sizeof(char));
    strcpy(ret->type, r->type);
    int pos = 0;
    ret->next = NULL;
    return ret;
}
